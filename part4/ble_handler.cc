...
const int DESIRED_ATT_MTU = 131; // bytes
...
void ble_start(){
  ...
  ble_cfg_t name_cfg = {
    .gap_cfg = {
      .device_name_cfg = {
        ...
        .current_len = (uint16_t)const_strlen(devicename),
        .max_len = (uint16_t)const_strlen(devicename)
      }
    }
  };
  ...
  sprintf(str,"Initial handle before install is %d\n",hndl);
  INFO(str);
...
void ble_handle_events(){
    ...
    while (NRF_ERROR_NOT_FOUND != sd_ble_evt_get(NULL, &event_len)){
      ...
      case BLE_GATTS_EVT_EXCHANGE_MTU_REQUEST:
        ...
        int chdl = event->evt.gatts_evt.conn_handle;
        sprintf(str,"Incoming request was %d\n",req_mtu);
        INFO(str);
        uint16_t resp_mtu = (req_mtu > DESIRED_ATT_MTU) ? DESIRED_ATT_MTU : req_mtu;
        sprintf(str,"We respond with %d\n",resp_mtu);
        INFO(str);
        int err;
        while (NRF_ERROR_BUSY == (err=sd_ble_gatts_exchange_mtu_reply(chdl,resp_mtu))) INFO("Had to retry MTU response");
        if (err != NRF_SUCCESS){
          printf("Failed to respond to gatt exchange: ");
          print_err(__FILE__,__LINE__,err);
        }else{
